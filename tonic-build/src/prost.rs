...
impl crate::Method for Method {
    ...
    fn request_response_name(
        ...
        let request = if (is_google_type(&self.inner.input_proto_type) && !compile_well_known_types)
            || self.inner.input_type.starts_with("::")
        {
            self.inner.input_type.parse::<TokenStream>().unwrap()
        } else if self.inner.input_type.starts_with("crate::") {
            syn::parse_str::<syn::Path>(&self.inner.input_type)
                .unwrap()
                .to_token_stream()
        } else {
        ...
        let response = if (is_google_type(&self.inner.output_proto_type) && !compile_well_known_types)
            || self.inner.output_type.starts_with("::")
        {
            self.inner.output_type.parse::<TokenStream>().unwrap()
        } else if self.inner.output_type.starts_with("crate::") {
            syn::parse_str::<syn::Path>(&self.inner.output_type)
                .unwrap()
                .to_token_stream()
        } else {
